...
export default class Company {
    ...
    async updateEmployeeRole(employee, role) {
        const employeeId = this.getEmployeeById(employee);
        const roleId = this.getRolesByTitle(role)[0].id;
        const[rows, fields] = this.updateEmployee("role_id", roleId, "id", employeeId);
        return rows;
    }

    async updateEmployeeManager(employee, manager) {
        const employeeId = this.getEmployeeById(employee);
        const managerId = this.getManagerId(manager);
        const [rows, fields] = this.updateEmployee("manager_id", managerId, "id", employeeId);
        return rows;
    }
    ...
    async updateEmployee(field, value, conditionField, conditionValue) {
        ...
        const [rows, fields] = await this.sql.updateMatchingEntry(
            'employees',
            field,
            value,
            conditionField,
