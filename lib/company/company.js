...
export default class Company {
    ...
    async getAllRoles() {
        const sql = await this.getConnection();
        const [rows, fields] = await sql.execute('SELECT roles.id AS "ID", title AS "Title", departments.dept_name AS "Department", salary AS "Salary" FROM roles INNER JOIN departments ON roles.department_id = departments.id;');
        await sql.end();
        return rows;
    }
}