...
export default class Company {
    ...
    async viewDepartmentBudget(department) {
        const departmentId = await this.getDepartmentId(department);
        const roles = (await this.getEntry("roles", "department_id", departmentId))[0];
        const departmentRoles = roles.map(element => element.id);
        let roleQuery = "";
        for (const id of departmentRoles) (
            roleQuery += '?, '
        )
        roleQuery = roleQuery.substring(0, roleQuery.length - 2);

        const [rows, fields] = this.sql.newQuery(
            `SELECT SUM(salary) As 'Budget' FROM roles INNER JOIN employees WHERE role_id IN (${roleQuery})`,
            [departmentRoles]
        );
