...
export default class RoleInterface extends DBI {
    ...
    async deleteRole(role) {
        const roleId = await this.getRole(role);
        if (!(roleId)) {
            return null;
        }
    ...
    async getAll() {
        return await this.sql.getAllEntries(this.table);
    }
    ...
    async #getDepartment(department) {
        ...
        const department = deptInterface.getDepartment(department);
    ...
    async #getDepartmentFromId(id) {
        const deptInterface = new DI(this.sql);
        let department = deptInterface.get('id', id);
        await deptInterface.close();
        if (department.length === 0) {
