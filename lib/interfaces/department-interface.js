...
export default class DepartmentInterface extends DBI {   
    ...
    async delete(department) {
        ...
        console.log(`\n${department} deleted successfully. Updated list of all departments:\n`);
    ...
    async viewBudget(department) {
        ...
        const [rows, fields] = await this.newQuery(
            `SELECT SUM(roles.salary) As 'Budget' FROM employees INNER JOIN roles ON role_id = roles.id AND roles.department_id = ?;`, 
            [departmentId]
        );
