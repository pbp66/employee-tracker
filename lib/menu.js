...
export default class Menu {
    ...
    async employeeQuery(method, values) {
        switch(method) {
            ...
            case('get'):
                const id = await this.employeeConn.getId(values[0]);
                return (await this.employeeConn.getEmployee(id)).view();
            case('delete'):
                    ...
                    case('add'):
                        questions.push(new Question(
                            ...
                            choices: async function (answer) {
                                const dept = await this.roleConn.getDepartment(
                                    answer['Role']
                                );
                                const list = (await 
                                this.employeeConn.getEmployeesByDepartment(
                                    dept
                                )).map(element => {
                                    return {
